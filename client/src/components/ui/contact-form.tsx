...
export function ContactForm() {
  ...
  const onSubmit = async (data: FormValues, event?: React.BaseSyntheticEvent) => {
    try {
      ...
      // Formspree submission using FormData
      const response = await fetch('https://formspree.io/f/xbdrwvgk', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = error.error || (error.errors ? error.errors.map((e: any) => e.message).join(', ') : errorMessage);
        } catch {
          // If JSON parsing fails, use fallback or status text
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel htmlFor="firstName">{t('contact.form.firstName')} *</FormLabel>
                <FormControl id="firstName">
                  <Input autoComplete="given-name" placeholder={t('contact.form.firstName')} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel htmlFor="lastName">{t('contact.form.lastName')} *</FormLabel>
                <FormControl id="lastName">
                  <Input autoComplete="family-name" placeholder={t('contact.form.lastName')} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl id="email">
                  <Input autoComplete="email" placeholder={t('contact.form.email')} type="email" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl id="phone">
                  <Input autoComplete="tel" placeholder={t('contact.form.phone')} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel htmlFor="canton">{t('contact.form.canton')} *</FormLabel>
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl id="canton">
                    <SelectTrigger name="canton">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel htmlFor="source">{t('contact.form.source')}</FormLabel>
                <Select
                  onValueChange={field.onChange}
                  defaultValue={field.value}
                >
                  <FormControl id="source">
                    <SelectTrigger name="source">
                      <SelectValue placeholder={t('please select')} />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
